...
-- Optional: prevent accidental deletes by default (no DELETE policy created)
-- Optional: INSERT handled by signup trigger/function
...
-- Keep existing policies and add an additional permissive policy
CREATE POLICY IF NOT EXISTS "messages_select_admin_staff"
ON public.messages
FOR SELECT
USING (is_admin(auth.uid()) OR is_staff(auth.uid()));

-- 3) MESSAGE_REPLIES: allow admins and staff to view all replies
CREATE POLICY IF NOT EXISTS "message_replies_select_admin_staff"
